            ...
            <section className='pl-2 flex space-x-5'>
                <div className='relative size-fit'>
                    <svg width="35" height="44" viewBox="0 0 35 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0.421875 6C0.421875 2.68629 3.10817 0 6.42188 0H21.8357L34.8964 13V38C34.8964 41.3137 32.2101 44 28.8964 44H6.42187C3.10817 44 0.421875 41.3137 0.421875 38V6Z" fill="#E86339"/>
                        <path d="M21.8359 13V0L34.8956 13H21.8359Z" fill="#FFB37C"/>
                    </svg>
                    <small className=' font-bold absolute top-5 left-1/2 -translate-x-1/2  '>{companyPPT?.type==="application/pdf"?"PDF":"PPT"}</small>
                </div>
                <aside className='grid'>
                    <small>{companyPPT?.name}</small>
                    <small className='text-[#B5B6B7]'>{convertFileSize(companyPPT?.size)} , {formatDate(companyPPT?.lastModified)}</small>
                </aside>
            </section>
            <button className='flex items-center text-[#FC4646] text-xs ' onClick={()=>dispatch({type:"ABOUT_COMPANY_FIELDS",payload:{inputField:"companyPPT",userInput:""}})}>
...
{
    const {state:{aboutCompany:{companyPPT}}, dispatch} = useProfileData();
    
    return (
        <section className='flex flex-col space-y-2.5'>
            <h1 className='text-xs font-bold'>Upload Company Presentation <span className='text-success font-normal'>(optional but recommended)</span></h1>
            <aside className={`flex min-h-20 border p-3 border-dashed ${companyPPT===""?"bg-transparent":"bg-bg-light"} border-white-darkest rounded-2xl`}>
            
                {companyPPT===""
                ?(
                <label htmlFor='presentation' className='flex justify-center items-center  hover:cursor-pointer grow'>
                    <input id='presentation' className='sr-only' type='file' accept=".ppt, .pptx, application/pdf" onChange={(e)=>dispatch({type:"ABOUT_COMPANY_FIELDS",payload:{inputField:"companyPPT",userInput:e.target.files[0]}})} />
                    <aside className='flex items-center space-x-2'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 10.5v6m3-3H9m4.06-7.19-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z" />
                        </svg>
                        <p className='text-sm'>Upload PPT/PDF</p>
                    </aside>
                </label>
                )
                :<Attachment companyPPT={companyPPT} dispatch={dispatch}/>
                }
            
            </aside>
            <small className='text-[#AAA6B9]'>Upload files in PPT/PDF format up to 50 MB.</small>
        </section>
    )
}

const AboutCompany = ({setStep}) => {
    const {state:{aboutCompany:{about,websiteurl}}, dispatch} = useProfileData();
