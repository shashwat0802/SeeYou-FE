  ...
  return (
    <section className="grid grid-cols-2 -mx-2.5">
      <label htmlFor="openPause" className="flex flex-col items-center">
        ...
        />
        <h1 className=" peer-checked:text-white text-[#35383F] shrink-0 text-lg  mb-3">
          Open & Paused(4)
        </h1>
      ...
      <label htmlFor="expiredClosed" className="flex flex-col items-center">
        ...
        />
        <h1 className=" peer-checked:text-white text-[#35383F] text-lg  mb-3">
          Expired/Closed(0)
        </h1>
        <aside className=" self-stretch bg-[#35383F] peer-checked:bg-white rounded-full mt-[1px] h-0.5 peer-checked:h-1 peer-checked:mt-0"></aside>
      </label>
    ...
    <section className="flex justify-between relative">
      ...
      <button className="group outline-none text-left">
        <svg
          ...
          strokeWidth={2.3}
          stroke="currentColor"
          className="size-4 text-white group-focus:text-success"
        >
          <path
            strokeLinecap="round"
...
const TagSelector = ({ tag }) => {
  switch (tag) {
    case 'Almost published':
      return (
        <span className="bg-white text-xs underline text-primary p-1.5 rounded-lg font-medium">
          {tag}
        </span>
      );
    ...
    case 'In Review':
      return (
        <span className="text-xs text-[#2897FF] bg-[#2897FF2E] p-1.5 rounded-lg tracking-wide">
          {tag}
        </span>
      );
    ...
    case 'Expiring in 3 days':
      return (
        <span className="text-xs text-required bg-[#FF616D2E] p-1.5 rounded-lg tracking-wide underline">
          {tag}
        </span>
      );
        ...
        <li key={jobPost?.jobId} className="p-4 border-b border-[#35383F]">
          <label htmlFor={jobPost?.jobId} className="flex items-center gap-x-4">
            ...
            <section className="grow">
              ...
              <p className="text-xs tracking-wide text-white-medium">
                {jobPost?.location}
                <span
                  className={`empty:hidden underline ml-1 ${
                    jobPost?.type === 'OPEN'
                      ? ' text-success'
                      : jobPost?.type === 'DRAFT'
                      ? 'text-required'
  ...
  return (
    <div className="mx-6 pt-4 space-y-6 pb-10">
      ...
      <aside className="px-4 py-3 rounded-xl space-x-4 flex items-center bg-primary-light">
        ...
        <input
          className="outline-none grow bg-transparent text-sm"
